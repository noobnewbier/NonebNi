...
void ShadowColor_float(float3 baseColor, float3 normalWs, float3 positionWs, out float3 finalColor)
{
    ...
    float nDotL = max(0.0, dot(normalWs, lightDirection));
    float halfLambert = pow(nDotL * 0.5 + 0.5, 2.);
    float3 halfLambertDiffuse = halfLambert * baseColor;

    finalColor = halfLambertDiffuse * lightAttenuation * lightColor;
}
